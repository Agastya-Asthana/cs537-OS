...
cowuvm(pde_t *pgdir, uint sz)
{
    ...
    lcr3(PADDR(pgdir));                     // Flush TLB for original process
    return d;
...
pagefault(uint err_code)
{
    ...
    if(va >= USERTOP || (pte = walkpgdir(proc->pgdir, (void*)va, 0)) == 0  ||
       !(*pte & PTE_P) || !(*pte & PTE_U) ){
        cprintf("Illegal virtual address on cpu %d addr 0x%x, kill proc %s with pid %d\n",
                cpu->id, va, proc->name, proc->pid);
    ...
    uint refCount = get_ref_count(pa);
    char *mem;
    ...
    if(refCount > 1) {
        ...
        //*pte =  (uint)walkpgdir(proc->pgdir, mem, 0) | PTE_P | PTE_U | PTE_W;
        *pte =  PADDR(mem) | PTE_P | PTE_U | PTE_W;
