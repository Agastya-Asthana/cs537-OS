...
void* request_handler(void* args) { 
    ...
    char cmd[10]; 
    strcpy(cmd, &req_args->request[0]);
    client_id = atoi(&req_args->request[1]);
    seq_number = atoi(&req_args->request[2]);
    arg1 = atoi(&req_args->request[3]);
    arg2 = atoi(&req_args->request[4]);
    ...
    send_packet(rpc.recv_socket, rpc.dst_addr, rpc.dst_len, response, strlen(response)); 
    free(req_args); 
    return NULL; 
} 
int main(int argc, char* argv[]) { 
    ...
    struct rpc_connection rpc;
    rpc.recv_socket = init_socket(port); 
    ...
    while (1) { 
        ...
        req_args->rpc = rpc; 
        req_args->rpc.recv_socket = rpc.recv_socket;
